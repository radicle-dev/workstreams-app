...
export const authStore = (() => {
	...
	async function authenticate(walletData: WalletData): Promise<AuthData> {
		...
		const signature = await walletData.signer.signMessage(message.prepareMessage());

		const result = await sendSignatureForVerification(message.prepareMessage(), signature);
