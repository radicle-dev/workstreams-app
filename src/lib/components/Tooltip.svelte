<script lang="ts">
	...
	function calculateOffset(position: Position, container: DOMRect, message: DOMRect): Offset {
		switch (position) {
			case 'top':
				return {
...
{#if value}
	<div {style} bind:this={container} data-cy="tooltip" on:mouseenter={show} on:mouseleave={hide}>
		<slot />
		<div
			bind:this={message}
